...
Die Transaktionstabelle (`trans`) dominiert mit 1.06 Mio. Zeilen und 100.5 MB den Datensatz bei weitem. Alle anderen Tabellen zusammen belegen weniger als 1 MB. Sie ist damit die primäre Datenquelle für das spätere Feature Engineering.

Mehrere Datumsspalten (`account.date`, `loan.date`, `trans.date`) liegen noch im Format YYMMDD als Integer vor und werden im nächsten Schritt in `datetime` konvertiert. Das Feld `card.issued` ist als String gespeichert und enthält teils angehängte Stunden.

Die `district`-Tabelle enthält kryptische Spaltennamen (A1 bis A16). Die für das Modell relevanten Felder sind Durchschnittsgehalt (A11: 8110 bis 12541 CZK), Arbeitslosenquote (A13: 0.4% bis 9.4%) und Urbanisierungsgrad (A10: 33.9% bis 100%). Die starke Streuung der Einwohnerzahl (A4: 42821 bis 1204953) zeigt den Grössenunterschied zwischen Prag und ländlichen Bezirken.

Zwei Auffälligkeiten aus `trans.describe()`:

* Kontostände können negativ werden (Minimum: 41125 CZK im Minus), d.h. Überziehungskredite sind im Datensatz vorhanden.
* Transaktionsbeträge sind stark rechtsschief (Median 2100 CZK gegenüber Mittelwert 5924 CZK), was auf vereinzelte sehr grosse Transaktionen hindeutet.
...
7 von 8 Tabellen weisen weder Duplikate noch fehlende Werte auf. Die Datenqualität ist damit insgesamt hoch.

Einzige Ausnahme ist `trans` mit vier betroffenen Spalten: `operation` (183114 NaN, ca. 17%), `k_symbol` (481881 NaN, ca. 46%), `bank` (782812 NaN, ca. 74%) und `account` (760931 NaN, ca. 72%). Die hohen Anteile deuten auf strukturell bedingte Lücken hin und nicht auf zufällige Datenfehler. Die detaillierte Analyse folgt im nächsten Abschnitt.
...
92.6% der Konten (4167 von 4500) verwenden den monatlichen Kontoauszug. Wöchentliche (240) und transaktionsbasierte (93) Auszüge sind selten. Die Kontoauszugs-Frequenz ist ein explizit gefordertes Baseline-Feature und fliesst als Indikatorvariable ins Modell ein.

Die monatlichen Kontoeröffnungen zeigen ein klares Wachstumsmuster: von 1993 bis Mitte 1996 nimmt die Neukunden-Akquisition deutlich zu und erreicht ein Maximum von rund 135 neuen Konten pro Monat. Ab Ende 1996 flacht das Wachstum ab. Das letzte Konto wurde am 1997-12-29 eröffnet, d.h. 1998 gibt es keine neuen Konten im Datensatz. Dieser Wachstumsverlauf ist später relevant für die Interpretation der Kalenderzeit-Verteilung der Karten-Events.
...
Das Alter ist relativ zum Ende der Transaktionshistorie (Ende 1998) berechnet. Die Verteilung erstreckt sich von 11 bis 87 Jahre (Median 44, Mittelwert 44.8). Das Minimum von 11 Jahren zeigt, dass Minderjährige im Datensatz vorhanden sind. Diese Gruppe ist für das Kreditkartenmodell nicht relevant und wird im Studiendesign über den Alters-Cutoff von 21 Jahren ausgeschlossen.

Die Altersverteilung ist im Bereich 20 bis 60 Jahre annähernd gleichmässig, mit einem leichten Häufigkeitsabfall nach dem 60. Lebensjahr.

Die Geschlechterverteilung ist nahezu ausgeglichen (M: 2724, F: 2645). Ein systematischer Gender-Bias in der Kundenbasis ist nicht vorhanden. Beide Merkmale sind explizit geforderte Baseline-Features für das Modell.
...
Prag (district_id 1) dominiert die Kontoverteilung mit 554 Konten und ist damit mehr als dreimal so gross wie der zweitgrösste Bezirk (district_id 70 mit 152). Prag ist auch der Ausreisser im Durchschnittsgehalt (~12500 CZK gegenüber dem Median von 8814 CZK aller Bezirke).

Das Durchschnittsgehalt (A11) ist rechtsschief verteilt. Die grosse Mehrheit der Bezirke liegt zwischen 8000 und 9500 CZK. Die Arbeitslosenquote (A13) ist ebenfalls rechtsschief mit einem Median von ca. 3.6%.
...
Nur 682 von 4500 Konten (15.2%) haben überhaupt einen Kredit. Kreditvergaben beginnen erst ab 1994 und nehmen bis 1998 zu, was wiederum das allgemeine Kundenwachstum widerspiegelt.

Die Laufzeiten sind auf fünf diskrete Werte standardisiert (12, 24, 36, 48, 60 Monate) und gleichmässig verteilt. Die 60-Monats-Laufzeit ist mit 144 Krediten leicht am häufigsten.

Der Grossteil der Kredite ist in Ordnung: Status C (laufend OK) macht 59% aus, Status A (abgeschlossen OK) weitere 30%. Die kombinierte Ausfallrate aus Status B (abgeschlossen, unbezahlt) und D (laufend, in Schulden) liegt bei rund 11%.

Aus dem Boxplot ist erkennbar, dass laufende Kredite mit Problemen (Status D) im Median deutlich höhere Beträge aufweisen als abgeschlossene Kredite. Ob ein Konto einen Kredit hat sowie die Kredithöhe und monatliche Rate sind Features für das Affinitätsmodell.
...
`household_payment` dominiert mit 3502 Einträgen (54%) klar. Die zweithäufigste Kategorie (1379 Einträge) ist im Plot nicht beschriftet, entspricht aber `payment_for_statement`. Insgesamt decken die fünf Kategorien household_payment, payment_for_statement, loan_payment (717), insurance_payment (532) und leasing (341) alle 6471 Daueraufträge ab.

Die Betragsverteilung variiert je nach Zweck: Kreditraten-Daueraufträge haben den höchsten Median (~4000 CZK), während Versicherungszahlungen tiefer liegen (~1500 CZK). Alle Kategorien weisen Ausreisser nach oben auf. Der Gesamtbereich ist 1 bis 14882 CZK bei einem Median von 2596 CZK.

Für das Modell relevant sind das Vorhandensein bestimmter Dauerauftragstypen als binäre Features (`has_household_order`, `has_loan_order`, `has_insurance_order`) sowie die Anzahl und Gesamtsumme aller Daueraufträge als Indikatoren für finanzielle Verpflichtungen.
...
Von 892 Karten total sind 145 Junior-Karten (16.2%), die gemäss Aufgabenstellung aus der Zielgruppe ausgeschlossen werden. Die verbleibenden 747 Karten (classic: 659, gold: 88) bilden die Positivklasse (Target = 1).

Die Kartenausstellungen konzentrieren sich stark auf die Jahre 1997 bis 1999. Vor 1996 wurden kaum Karten vergeben. Diese zeitliche Konzentration erklärt, warum die Käufer-Gruppe im Studiendesign eine spezifische Kalenderzeit-Verteilung aufweist und ist der Hauptgrund dafür, dass das Tenure-Matching gegenüber einem reinen Kalenderzeit-Ansatz bevorzugt wird.

Gold-Karten sind selten (88, entspricht 11.8% der gültigen Karten) und treten erst ab 1996/97 regelmässig auf. Classic-Karten wachsen ab 1997 stark und erreichen Ende 1998 einen Peak von rund 40 Ausstellungen pro Monat.
...
Die Verteilung ist rechtsschief: Die Mehrheit der Kartenausstellungen erfolgt innerhalb der ersten 2 Jahre nach Kontoeröffnung, mit einem Median von 22 Monaten und einem Modus um 20 Monate. Ein langer rechter Schwanz erstreckt sich bis 71 Monate.

Das Minimum von 6 Monaten zeigt, dass einzelne Karten sehr früh ausgestellt wurden. Der 25%-Quantil liegt bei 13 Monaten, was dem Mindest-Rollup-Constraint (12M Rollup + 1M Lag = 13M) entspricht. Karten, die innerhalb der ersten 13 Monate ausgestellt wurden, können kein vollständiges Rollup-Fenster vorweisen und werden im Studiendesign ausgeschlossen.

Diese Verteilung bildet die empirische Grundlage für die Pseudo-Event-Zuweisung an Nicht-Kartenbesitzer. Sie wird ausschliesslich aus Train-Card-Holders berechnet, um Data Leakage zu verhindern.
...
Die Zielvariable ist moderat unbalanciert: 13.9% Kartenbesitzer (747) gegenüber 86.1% Nicht-Kartenbesitzer (4622). Dieser Vergleich erfolgt noch vor dem Rollup-Filter und ohne Studiendesign-Bereinigung.

Die Unterschiede zwischen den Gruppen auf den Baseline-Features sind insgesamt gering:

* **Alter:** Kartenbesitzer sind im Schnitt etwas jünger (Mittelwert 43.0 vs. 45.1 Jahre). Der Unterschied ist statistisch vorhanden, aber klein.
* **Geschlecht:** Kartenbesitzer sind leicht häufiger männlich (53% vs. 50%). Kein starkes Signal.
* **Durchschnittsgehalt:** Marginaler Unterschied (9605 vs. 9496 CZK). Beide Gruppen ähnlich verteilt.
* **Kredit vorhanden:** Der deutlichste Unterschied: 20% der Kartenbesitzer haben einen Kredit gegenüber 15% der Nicht-Kartenbesitzer. Kartenbesitzer nutzen tendenziell mehr Finanzprodukte.
* **Kontoauszugs-Frequenz:** Nahezu identisch in beiden Gruppen, kein Signal.

Die schwache Trennung auf den Basis-Features zeigt, dass diese allein nicht ausreichen für ein starkes Modell. Transaktionsbasierte Behavioral-Features aus dem Rollup-Fenster werden für die eigentliche Trennleistung entscheidend sein.
...
# assign_pseudo_event_dates ist in src/data.py definiert (DRY-Prinzip).
